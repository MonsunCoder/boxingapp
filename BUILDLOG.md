...
Days 38-39 - Paywall + Access & Funding (double day) - August 19-20
		The entitlement is the law now (standing rule 6): is_premium() reads the subscriptions table server-side; every paid feature checks it, never a client flag. The A6 paywall is built to its own decisions — transparent two-column free-vs-Pro, three price cards ($3.99/mo, $29.99/yr with 14-day trial badge, $99.99 lifetime), and "Keep training free" at EQUAL visual weight to the trial button. Purchases are honestly stubbed ("coming with the store build") — RevenueCat products, sandbox purchases, and the webhook that writes source='store' rows all need the EAS build + store accounts: pre-launch list. Day 39 is fully LIVE though: the Access & Funding Program — redeem_code() grants the SAME premium (source 'code', no second-class markers), batch generation via generate_access_codes() (HOPE-XXXXXX, mod-only), org seat pools tracked (org_seat_usage), demo org seeded with test code HOPE-TEST01 (5 seats, 90 days). Scholarship loop dry-run verified: redeem -> premium true, duplicate blocked, bad code rejected. Wired: Meal Plans Pro card, Account row, and the A6 moment — a "see what's free vs Pro" link on a first-time workout summary (function-over-flow placeholder for the strict step-8 onboarding placement). Day 10's schema vocabulary held again: sources store/code/org, orgs.seat_pool.
