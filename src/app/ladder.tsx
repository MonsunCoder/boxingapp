 ...
 *
 * Day 22 layers the ethics gate on top: required_ethics_ids gets filled in and
 * the "Words first" group below starts appearing in the checklist.
 */
...
export default function LadderScreen() {
  ...
  const rankUp = async () => {
    ...
    if ((result.blocked_on_ethics ?? []).length > 0) {
      // Day 22 turns this into the real words-first prompt with a link to the lesson.
      setMessage('Words first. There is a conflict lesson to finish before this belt.');
      return;
    }
        ...
        <View style={styles.nextCard}>
          ...
          {groups.map((g) => (
            <View key={g.group} style={styles.group}>
              ...
              {g.items.map((item) => (
                <View key={item.content_id} style={styles.reqRow}>
          ...
          <Pressable
            ...
            disabled={!can_rank_up || busy}
            onPress={rankUp}>
            <Text style={styles.rankBtnText}>
              {busy ? 'Checking…' : can_rank_up ? `Rank up to ${rankLabel(next, tiers)}` : 'Finish the list to rank up'}
            </Text>
          </Pressable>
